...
pub fn main() !void {
    const maxRedCubes: i32 = 12;
    _ = maxRedCubes;
    const maxGreenCubes: i32 = 13;
    _ = maxGreenCubes;
    const maxBlueCubes: i32 = 14;
    _ = maxBlueCubes;
    ...
    while (line != null) {
        var gameId: i32 = 0;
        var minRedCubes: i32 = 0;
        var minBlueCubes: i32 = 0;
        var minGreenCubes: i32 = 0;
        const delimiters = [_]u8{ ':', ',', ';' };
        var compontes = std.mem.splitAny(u8, line.?, &delimiters);
        var comp: ?[]const u8 = compontes.first();

        while (comp != null) {
            if (std.mem.containsAtLeast(u8, comp.?, 1, "Game")) {
                gameId = try ExtractNumber(comp.?);
            } else if (std.mem.containsAtLeast(u8, comp.?, 1, "red")) {
                const number = try ExtractNumber(comp.?);
                if (number > minRedCubes) {
                    minRedCubes = number;
                }
            } else if (std.mem.containsAtLeast(u8, comp.?, 1, "green")) {
                const number = try ExtractNumber(comp.?);
                if (number > minGreenCubes) {
                    minGreenCubes = number;
                }
            } else if (std.mem.containsAtLeast(u8, comp.?, 1, "blue")) {
                const number = try ExtractNumber(comp.?);
                if (number > minBlueCubes) {
                    minBlueCubes = number;
                }
            }
        ...
        const power: i32 = minRedCubes * minGreenCubes * minBlueCubes;
        sum += power;
...
fn ExtractNumber(string: []const u8) !i32 {
    var numberStr: [64]u8 = undefined;
    var i: usize = 0;
    var j: usize = 0;
    ...
    const numberSlice = numberStr[0..j];
    const gameId = try std.fmt.parseInt(i32, numberSlice, 10);
    return gameId;
}
